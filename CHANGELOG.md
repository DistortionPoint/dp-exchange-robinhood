...
- **A forwarded `nil` option was used as the value instead of meaning "not set".** This
  ...
  **`:limiter`** returned `nil` from `Supervisor.limiter_name/1`: the limiter started
  ...
  a different, usually absent limiter. **`:subscriber`** and `:interval_ms` on the feed are
...
- **`Fake.subscribe/2` replaced the live subscription instead of adding to it.** It was the
  ...
  One of this package's own tests was pinning the defect. "subscribe works without options,
  and an unlisted symbol pushes nothing" asserted `coverage() == %{}` after subscribing a
  LISTED symbol and then an unlisted one — and passed only because the second call wiped the
  first. The empty map came out of the bug, not out of the property under test. It now asserts
  the listed symbol IS covered and the unlisted one is not, which holds whichever way
  `subscribe/2` composes.
...
Backfilled by hand on 2026-09-13 — see dp-exchange-coinbase issue #2, which reported the
identical shape here. The heading was absent from the published tarball because
`## [Unreleased]` was empty at release time and the pipeline skipped the cut. **The 0.3.20
tarball on Hex cannot be changed and still has no heading**; this entry makes the record
complete from the next release onward.
...
- **`"NaN"` and `"Inf"` from a venue became real `Decimal` prices and flowed through
  ...
  **Found where it was fixed, not where it applied.** `dp_exchange_webull` hit this and
  guarded both of its own copies; the other four venues guarded **none at all — this one had 1**. Every
...
- **A dead subscriber's pid was never removed, and the fan-out walked it on every message
  ...
  **This venue prunes `notice_subscribers` only.** `state.subscriber` — the data consumer —
  is configuration rather than a subscription: it arrives in `start_link/1`'s opts and no
  call can change it. Dropping it when its holder dies would leave a feed that can never
  deliver again with nothing able to repair it short of restarting the tree, and it cannot
  leak either, being one pid rather than a set that accumulates.
## [0.3.8] - 2026-09-11
...
- **A re-capture command in `endpoint-inventory.md` broke when followed literally.** It
  wrote its placeholder in angle brackets — the docs host followed by `<that bundle>`. In
  prose that reads fine; pasted into a shell, `<` is an input redirect, so the command
  silently fetched the bare host with its stdin pointed at a file called `that` and never
  touched the bundle it was supposed to read. It now uses shell variables and runs end to
  end.

  Found by the coverage check above, which reported the mangled string as a cited URL on an
  unrecognised host — it was looking for unverified documentation sources and turned up a
  broken command instead. The first fix replaced the placeholder with `BUNDLE_PATH`, and the
  same check immediately reported **that** as a cited-but-unlisted documentation URL. It was
  right to: a URL-shaped placeholder is still an unfollowable URL, and the fix had moved the
  defect rather than removed it.

- **The documentation root is now a checked source.** This venue publishes exactly one
  documentation page, so its manifest listed exactly one URL — the thinnest evidence base in
  the family. `https://docs.robinhood.com` 307s to that page, and if that redirect ever
